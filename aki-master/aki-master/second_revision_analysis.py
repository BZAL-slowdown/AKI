r"""Second-revision clinical baseline, calibration, decision-curve, and robustness analysis.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_dir / "second_revision_results",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Repeated stratified CV repeats for robustness checks using modal tuned parameters.",
    )
...
def threshold_metrics(y_true: np.ndarray, prob: np.ndarray, threshold: float = 0.5) -> dict:
    ...
    return {
        ...
        "mcc_at_0.5": matthews_corrcoef(y_true, pred),
    }


def probability_metrics(y_true: np.ndarray, prob: np.ndarray) -> dict:
    out = {
        "auc": safe_auc(y_true, prob),
        "average_precision": average_precision_score(y_true, prob),
        "brier": brier_score_loss(y_true, prob),
    }
    out.update(threshold_metrics(y_true, prob))
    return out
...
def run_nested_cv(
    ...
    for fold, (train_idx, test_idx) in enumerate(outer_cv.split(x, y), start=1):
        ...
        for model_name, grid in models.items():
            ...
            metric_row.update(probability_metrics(y_test, prob))
...
def modal_params_by_model(fold_metrics: pd.DataFrame) -> dict[str, dict]:
    modal_params = {}
    for model_name, group in fold_metrics.groupby("model", sort=False):
        serialized = group["best_params"].tolist()
        winner = Counter(serialized).most_common(1)[0][0]
        modal_params[model_name] = json.loads(winner)
    return modal_params


def run_repeated_cv_robustness(
    x: pd.DataFrame,
    y: np.ndarray,
    inner_folds: int,
    repeats: int,
    ...
    model_grids = build_models(inner_folds)
    ...
    for split_id, (train_idx, test_idx) in enumerate(cv.split(x, y), start=1):
        ...
        for model_name, grid in model_grids.items():
            estimator = clone(grid.estimator)
            ...
            row = {
                ...
                "n_positive": int(y_test.sum()),
                "n_negative": int((y_test == 0).sum()),
                "fixed_params_from_primary_cv": json.dumps(tuned_params[model_name], ensure_ascii=False),
            }
            row.update(probability_metrics(y_test, prob))
            rows.append(row)
    ...
    for model_name, group in metrics.groupby("model", sort=False):
        ...
        for col in metric_cols:
            mean = group[col].mean()
            sd = group[col].std(ddof=1)
            se = sd / math.sqrt(len(group)) if len(group) else math.nan
            row[f"{col}_mean"] = mean
            row[f"{col}_sd"] = sd
            row[f"{col}_approx_95ci_low"] = mean - 1.96 * se if not math.isnan(se) else math.nan
            row[f"{col}_approx_95ci_high"] = mean + 1.96 * se if not math.isnan(se) else math.nan
...
def summarize_predictions(predictions: pd.DataFrame, n_bootstrap: int) -> pd.DataFrame:
    ...
    for model_name, group in predictions.groupby("model", sort=False):
        ...
        thresh = threshold_metrics(y_true, prob)
        ...
        rows.append(
            {
                ...
                "accuracy_at_0.5": thresh["accuracy_at_0.5"],
                "balanced_accuracy_at_0.5": thresh["balanced_accuracy_at_0.5"],
                "precision_at_0.5": thresh["precision_at_0.5"],
                "recall_at_0.5": thresh["recall_at_0.5"],
                "sensitivity_at_0.5": thresh["recall_at_0.5"],
                ...
                "f1_at_0.5": thresh["f1_at_0.5"],
                "mcc_at_0.5": thresh["mcc_at_0.5"],
                "tp_at_0.5": int(tp),
                "fp_at_0.5": int(fp),
                "tn_at_0.5": int(tn),
                "fn_at_0.5": int(fn),
...
def save_notes(
    ...
    best_robust = robustness_summary.sort_values("auc_mean", ascending=False).iloc[0]
    ...
    notes = [
        ...
        f"- Robustness check: repeated stratified 5-fold CV used each model's modal hyperparameters from the primary nested CV. The best mean repeated-CV AUC was {best_robust['model']} at {best_robust['auc_mean']:.3f} +/- {best_robust['auc_sd']:.3f} SD.",
...
def main() -> None:
    ...
    tuned_params = modal_params_by_model(fold_metrics)
    robustness_metrics, robustness_summary = run_repeated_cv_robustness(
        x=x,
        y=y,
        inner_folds=args.inner_folds,
    ...
    pd.DataFrame(
        [
            {
                "model": model,
                "modal_params_from_primary_cv": json.dumps(params, ensure_ascii=False),
            }
            for model, params in tuned_params.items()
        ]
    ).to_csv(paths.output_dir / "modal_hyperparameters.csv", index=False, encoding="utf-8-sig")
    robustness_metrics.to_csv(paths.output_dir / "robustness_repeated_cv_metrics.csv", index=False, encoding="utf-8-sig")
    robustness_summary.to_csv(paths.output_dir / "robustness_repeated_cv_summary.csv", index=False, encoding="utf-8-sig")
    ...
    save_notes(paths.output_dir, audit, features, summary, robustness_summary, image_audit, args.feature_set)

    print(f"Wrote second-revision baseline outputs to: {paths.output_dir}")
    print("Primary nested-CV pooled out-of-fold summary:")
    print(summary[["model", "auc", "auc_95ci_low", "auc_95ci_high", "brier", "mcc_at_0.5", "calibration_slope"]].to_string(index=False))
    print("\nRepeated-CV robustness summary:")
    print(robustness_summary[["model", "auc_mean", "auc_sd", "brier_mean", "mcc_at_0.5_mean"]].to_string(index=False))
